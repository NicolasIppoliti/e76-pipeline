...
For Northwind orders only, a blank `order_id`, negative `gross`, or currency different from the tenant's configured currency (USD) rejects that row into `pipeline.order_quarantine`, rather than failing the whole file. Other valid rows in the file still publish; rejected rows do not enter `staging.orders` or `reporting.daily_orders`. The `pipeline.files` record counts rejected rows separately as `rejected_count` alongside `inserted_count` and `duplicate_count`, and exact replay leaves both published orders and quarantine unchanged. Other tenants retain their existing invalid-order behavior. An authorized tenant can inspect its own rejected rows and their tenant, source file, physical CSV line, and reason using its tenant database login (not the provisioning credential):

```sql
SELECT tenant_id, source_file, line_number, reason
FROM pipeline.order_quarantine
ORDER BY source_file, line_number;
```

The mixed-quality Northwind `batch_06` fixture exists under `fixtures/` but is provisioned **only by integration tests**; it is not listed in the checked-in manifest. The normal CLI cannot ingest that batch until it is explicitly registered in the manifest and its fixture is supplied through the configured fixture path. This documentation does not register or deliver it.
